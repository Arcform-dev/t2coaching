...
// Ported from the shadcn `animated-card` to plain JSX and recolored for this
// site: the light/dark-mode card becomes a solid navy card, and the purple/amber
// chart defaults are replaced with the brand sky/gold via the Visual1 props.
// Internal widths stay at the design's 356px; `max-w-full` lets it shrink to fit
// (the visual clips gracefully on narrow screens).
...
export function AnimatedCard({ className, ...props }) {
  return (
    <div
      ...
      className={cn(
        'group/animated-card relative w-[356px] max-w-full overflow-hidden rounded-xl border border-white/10 bg-[#0A1E2B] shadow-lg',
        className
      )}
      {...props}
    />
...
export function Visual1({
  ...
  gridColor = 'rgba(126,200,227,0.10)',
}) {
  return (
...
const Layer1 = ({ color, secondaryColor }) => (
  <div className="ease-[cubic-bezier(0.6,_0.6,_0,_1)] absolute top-0 left-0 z-[6] transform transition-transform duration-500 group-hover/animated-card:translate-x-[-50%]">
...
const Layer2 = ({ color }) => (
  <div className="absolute top-0 left-[-1px] h-full w-[356px]">
    ...
    <div className="ease-[cubic-bezier(0.6,_0.6,_0,_1)] absolute inset-0 z-[3] transform bg-gradient-to-r from-transparent from-0% to-[#0A1E2B] to-15% transition-transform duration-500 group-hover/animated-card:translate-x-full"></div>
  </div>
)
...
const Layer3 = ({ color, secondaryColor }) => (
  <div
    ...
    style={{ '--color': color, '--secondary-color': secondaryColor }}
  >
    <div className="flex shrink-0 items-center rounded-full border border-white/15 bg-black/30 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
      <div className="h-1.5 w-1.5 rounded-full bg-[var(--color)]" />
      <span className="ml-1 text-[10px] text-white">Fitness</span>
    </div>
    <div className="flex shrink-0 items-center rounded-full border border-white/15 bg-black/30 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
      <div className="h-1.5 w-1.5 rounded-full bg-[var(--secondary-color)]" />
      <span className="ml-1 text-[10px] text-white">Fatigue</span>
    </div>
  </div>
)
...
const Layer4 = () => (
  <div className="group relative h-full w-[356px]">
    <div className="ease-[cubic-bezier(0.6,_0.6,_0,_1)] absolute inset-0 z-[7] flex max-w-[356px] -translate-y-full items-start justify-start bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
      <div className="ease-[cubic-bezier(0.6,_0.6,_0,_1)] rounded-md border border-white/15 bg-black/40 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100">
        <p className="mb-1 text-xs font-semibold text-white">Weekly Training Load</p>
        <p className="text-xs text-white/55">Your fitness is trending up.</p>
      </div>
    </div>
  </div>
)
