      ...
      <>
        <section style={{ padding: '160px 0 90px', textAlign: 'center' }}>
          <div style={WRAP}>
            ...
            <h1 style={{ fontFamily: "'Noto Sans', system-ui, -apple-system, sans-serif", fontSize: 'clamp(2rem, 5vw, 3.2rem)', color: '#fff', lineHeight: 1.2, margin: '18px 0 20px' }}>{post.title}</h1>
    ...
    <>
      ...
      <header style={{ position: 'relative', padding: '140px 0 40px', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, ...WRAP }}>
          ...
          <h1 style={{ fontFamily: "'Noto Sans', system-ui, -apple-system, sans-serif", fontSize: 'clamp(2.2rem, 5vw, 3.6rem)', color: '#fff', lineHeight: 1.15 }}>{post.title}</h1>
        </div>
      </header>
        ...
        <div style={WRAP}>
          ...
          {post.exercises && (
            <div style={{ marginTop: 40, display: 'flex', flexDirection: 'column', gap: 24 }}>
              <h2 style={{ fontFamily: "'Noto Sans', system-ui, -apple-system, sans-serif", fontSize: 28, color: '#fff', marginBottom: 4 }}>The Workout</h2>
              {post.exercises.map((ex, i) => (
                <Reveal key={ex.name} delay={(i % 2) * 0.05}>
                  <GlassCard style={{ padding: '28px 28px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
                      <span style={{ fontFamily: "'Noto Sans', system-ui, -apple-system, sans-serif", fontSize: 22, color: 'rgba(126,200,227,0.6)' }}>{String(i + 1).padStart(2, '0')}</span>
                      <h3 style={{ fontFamily: "'Noto Sans', system-ui, -apple-system, sans-serif", fontSize: 20, color: '#fff' }}>{ex.name}</h3>
                    </div>
